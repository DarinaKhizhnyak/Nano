...
public class Generation {
    ...
    /**
     * Конструктор класса Generation создающий объект Generation
    ...
    public ObservableList<Particle> ParticlesGeneration(ObservableList<Particle> particlesList) {
        ...
        for (int i = 0; i < numberOfParticle; i++) {
            Particle particle3D = new Particle(radius, COLORS[i % COLORS.length],
                    tube.getRadius()*Math.cos(list.get(i).getX()/tube.getRadius()),
                    tube.getRadius()*Math.sin(list.get(i).getX()/tube.getRadius()),
                    list.get(i).getY()-tube.getHeight()/2);
